...
class DWS_Sniffs_WhiteSpace_SuperfluousWhitespaceSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_OPEN_TAG) {
            ...
            $classClosePtr = $tokens[$stackPtr]['scope_closer'];
            $structureType = strtolower(str_replace('T_', '', $tokens[$stackPtr]['type']));
            if ($tokens[$classOpenPtr + 1]['content'] == $phpcsFile->eolChar && $tokens[$classOpenPtr + 2]['content'] == $phpcsFile->eolChar)
                $phpcsFile->addError("Blank line at beginning of $structureType", $classOpenPtr + 2, 'EmptyLines');
            if ($tokens[$classClosePtr - 1]['content'] == $phpcsFile->eolChar && $tokens[$classClosePtr - 2]['content'] == $phpcsFile->eolChar)
                $phpcsFile->addError("Blank line at end of $structureType", $classClosePtr - 1, 'EmptyLines');
        } else {
